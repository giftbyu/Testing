...
# Semua logika dari skrip pengujian sebelumnya kita masukkan ke dalam fungsi ini
def start_detection(metode):
    ...
    # Konfigurasi berdasarkan metode yang dipilih
    if metode == 'sibi':
    ...
    while cap.isOpened():
        ...
        # Pra-pemrosesan
        image_for_cnn = np.expand_dims(cv2.resize(frame, (128, 128)), axis=0).astype(np.float32)
        landmarks_vector = np.zeros(NUM_LANDMARKS, dtype=np.float32)

        if results.multi_hand_landmarks:
            # Logika SIBI (1 tangan)
            if metode == 'sibi' and len(results.multi_hand_landmarks) > 0:
                hand_landmarks = results.multi_hand_landmarks[0]
                mp_drawing.draw_landmarks(display_frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)
                coords = np.array([[lm.x, lm.y, lm.z] for lm in hand_landmarks.landmark])
                landmarks_vector = (coords - coords[0]).flatten()
            # Logika BISINDO (bisa 2 tangan)
            elif metode == 'bisindo':
        ...
        if image_input_index is not None and landmark_input_index is not None:
            interpreter.set_tensor(input_details[image_input_index]['index'], image_for_cnn)
            interpreter.set_tensor(input_details[landmark_input_index]['index'], landmarks_for_mlp)
            interpreter.invoke()
...
# --- Bagian 2: GUI Launcher dengan Tkinter ---
def main():
    # Fungsi ini akan dipanggil saat tombol di klik
    def run_detection(metode):
        root.destroy() # Tutup jendela launcher
        start_detection(metode) # Jalankan fungsi deteksi

    # Buat jendela utama
    root = tk.Tk()
    root.title("Pilih Metode Deteksi")
    
    # Atur ukuran jendela dan posisi di tengah layar
    window_width = 400
    window_height = 200
    ...
    # Frame untuk tombol agar bisa diletakkan berdampingan
    ...
    sibi_button = tk.Button(
        button_frame, 
        text="Mulai Deteksi SIBI", 
        font=button_font, 
        command=lambda: run_detection('sibi'),
    ...
    bisindo_button = tk.Button(
        button_frame, 
        text="Mulai Deteksi BISINDO", 
        font=button_font, 
        command=lambda: run_detection('bisindo'),
